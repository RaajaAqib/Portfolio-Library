...
async function renderIndex() {
    try {
        let activeCategory = sessionStorage.getItem('libraryCategory') || 'All';


        // -----------------------------
        // RESTORE CATEGORY FROM HASH
        // Expected: #library?category=ITSM
        // -----------------------------
    ...
    // STEP 3 – highlight
    const lastOpened = sessionStorage.getItem('lastOpenedPost');
    if (lastOpened === post.slug) {
        div.classList.add('last-opened');
    }

    // FIX 1 – manual link creation (ADD HERE)
  ...
  link.addEventListener('click', (e) => {
    ...
    sessionStorage.setItem('libraryCategory', activeCategory);
    sessionStorage.setItem('lastOpenedPost', post.slug);

    // ✅ NAVIGATE MANUALLY AFTER SAVE
    ...
    meta.textContent = `${post.date} · ${post.category} · ${post.tags.join(', ')}`;
...
// Save scroll, category, and last opened post before navigating
document.addEventListener('click', e => {
    ...
    const slug = new URL(link.href).searchParams.get('post');
    if (slug) {
        sessionStorage.setItem('lastOpenedPost', slug);
    }
});
